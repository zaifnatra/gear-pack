...
import { TripType, Difficulty, TripVisibility, NotificationType } from '@prisma/client'
...
/**
 * Membership guard used by the API layer: verifies the user is an accepted
 * participant of the trip before allowing trip-scoped mutations.
 */
export async function isTripParticipant(tripId: string, userId: string) {
    const participant = await prisma.participant.findUnique({
        where: {
